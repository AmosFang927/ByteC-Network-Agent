            ...
            query = f"""
                SELECT 
                    ...
                    SUM(COALESCE(
                        usd_sale_amount, 
                        sale_amount,
                        (raw_data->>'usd_sale_amount')::numeric,
                        (raw_data->>'sale_amount')::numeric,
                        0
                    )) as total_sales,
                    SUM(COALESCE(
                        usd_payout, 
                        payout,
                        (raw_data->>'usd_payout')::numeric,
                        (raw_data->>'payout')::numeric,
                        (raw_data->'original_data'->>'base_payout')::numeric,
                        (raw_data->'original_data'->>'bonus_payout')::numeric,
                        0
                    )) as total_payout,
                    AVG(COALESCE(
                        usd_sale_amount, 
        ...
        try:
            ...
            query = f"""
                SELECT 
                    ...
                    SUM(COALESCE(usd_sale_amount, sale_amount, 0)) as total_sales,
                    SUM(COALESCE(usd_payout, payout, 0)) as total_payout,
                    AVG(COALESCE(usd_sale_amount, sale_amount, 0)) as avg_sale_amount
                FROM conversions
                WHERE {' AND '.join(where_conditions)}
        ...
        try:
            ...
            data_query = f"""
                SELECT 
                    ...
                    -- 佣金计算字段
                    COALESCE(commission_rate, 1.0) as "commission_rate",
                    CASE 
                        WHEN COALESCE(usd_sale_amount, 0) > 0 AND COALESCE(commission_rate, 0) > 0 
                        THEN COALESCE(usd_sale_amount, 0) * COALESCE(commission_rate, 0) / 100
                        ELSE COALESCE(usd_payout, 0)

...
class DashboardService:
    ...
    def _process_summary_metrics(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ===== 添加前端期望的字段映射 =====
        # 前端期望: total_commission, avg_commission_rate
        # 后端提供: total_payout, profit_margin
        processed['total_commission'] = metrics.get('total_payout', 0)
        processed['avg_commission_rate'] = processed.get('profit_margin', 0)
        
        # 添加銷售金額的別名
        processed['total_sale_amount'] = metrics.get('total_sales', 0)

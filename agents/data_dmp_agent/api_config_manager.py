...
try:
    ...
    # 如果無法導入config，拋出錯誤，不應該使用hardcoded配置
    raise ImportError("無法導入config模組，請確保config.py文件存在且路徑正確")
...
class APIConfigManager:
    ...
    def __init__(self):
        self.configs = {
            ...
            'IATestByteC': {
                'name': 'Involve Asia Test ByteC',
                'base_url': config.IATESTBYTEC_BASE_URL,
                'api_key': config.IATESTBYTEC_API_KEY,
                'secret': config.IATESTBYTEC_API_SECRET,
                'endpoints': {
                    'conversions': '/conversions/range',
                    'auth': '/authenticate'
                },
    ...
    def validate_config(self, platform: str) -> bool:
        ...
        if not config:
            logger.error(f"❌ 平台 {platform} 配置不存在")
            return False
